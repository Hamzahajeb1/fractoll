...
/*   Updated: 2024/05/18 17:28:25 by hel-haji         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	event(t_fractal *fract)
{
	...
	mlx_hook(fract->mlx_window, ButtonPress, ButtonPressMask, handle_mouse,
		fract);
...
void	ft_init(t_fractal *fract)
{
	...
	fract->image.pixl = mlx_get_data_addr(fract->image.img,
			&fract->image.bit_pix, &fract->image.size_line,
			&fract->image.endian);
	event(fract);
	data(fract);
}
